...
void DrawAsteroid(const int & cx, const int &cy, const ColorNames & cname) // center of Asteroid
		{
	int radius = 50; // sample radius
	//int rndpoint = 7;//GetRandInRange(5, 9);
	int npoints = 6;//MAX(4, rndpoint);
	//cout << npoints << endl;
	...
	for (int i = 0; i < npoints - 1; ++i) {
		theta += 60;//GetRandInRange(stheta, stheta + dtheta);
		cout << stheta << " " << dtheta << endl;
